...
### Netty内存管理
```text
Netty内存管理是Netty框架的核⼼部分，也是Netty技术较难掌握的内容之⼀。Netty内存管理采⽤堆外内存分配的⽅式，
从⽽避免了频繁的垃圾回收（GC）操作，这也正是Netty内存管理设计的特殊之处。

1、内存管理基础
内存管理的主要⽬的：合理分配内存，尽量减少内存碎⽚，能够及时回收资源，提⾼内存的使⽤效率。

从操作系统的内存管理层⾯来讲，应⽤进程在运⾏时会向操作系统请求对内存资源的快速分配，并且在适当的时候释放和回收内存资源。
基于这个理念，主流操作系统均实现了各⾃的内存管理算法。

内存管理是⼀个⼗分复杂且很有难度的⼯作，⽬前基于内存管理实现了许多知名的算法。⽐如，为了核⼼内存管理能够快速响应请求的Buddy算法，Linux系统基于⼩块内存管理的slab算法，
以及最近⾮常流⾏的、基于FreeBSD系统的多线程管理⽽设计的jemalloc算法。⽽Netty框架的内存管理⽅式就是基于jemalloc算法⽽实现的。
